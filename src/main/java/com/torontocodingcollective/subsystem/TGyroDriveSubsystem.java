...
public abstract class TGyroDriveSubsystem extends TDriveSubsystem {
	 ...
	 *            that extends the {@link TMotorController}
	 * @param rightSpeedController
	 *            that extends {@link TMotorController}
	 * @param leftEncoder
	 ...
	 *            angle
	 */
	public TGyroDriveSubsystem(TMotorController leftSpeedController, TMotorController rightSpeedController,
	...
	private double setDriveOnHeadingSpeeds() {
		...
		if (Math.abs(angleError) > 30) {
			...
			if (Math.abs(leftSpeed) > maxRotationOutput) {
				leftSpeed = maxRotationOutput * Math.signum(leftSpeed);
			}
